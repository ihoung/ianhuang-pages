...
export default function ProjectsPreview({ locale, projects }: Props) {
  ...
  const lgColClass = projects.length >= 3 ? "lg:grid-cols-3" : projects.length === 2 ? "lg:grid-cols-2" : "lg:grid-cols-1";
  return <section id="projects" className="site-container min-h-screen flex flex-col justify-center py-20 md:py-28 snap-start snap-always" aria-label="Featured Projects">
    ...
    <div className={`grid grid-cols-1 sm:grid-cols-2 ${lgColClass} gap-6 md:gap-8 max-w-5xl`}>{projects.map((project) => <ProjectCard key={project.slug} title={project.title} description={project.description} thumbnail={project.thumbnail} href={`/${locale}/content/${project.slug}`} locale={locale} aspect="aspect-[16/10]" />)}</div>
